<?php  
  ...
  $sql_tema = $db->consulta("SELECT * FROM mandino_temas LEFT JOIN usuarios ON usuarios.fk_mt = mandino_temas.mt_id ORDER BY mt_id ASC");

  for ($i=0; $i < $sql_tema['cantidad_registros']; $i++) { 
    if($sql_tema[$i]['u_id'] == $usuario['id']){
